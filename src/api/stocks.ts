...
export async function getOrderBook(companyId: string): Promise<CompanyOrderBook> {
  const response = await apiClient.get<ApiResponse<CompanyOrderBook>>(`/order-book/${companyId}`);
  return response.data.data;
}

export async function getMarketData(companyId: string): Promise<CompanyMarket[]> {
  const response = await apiClient.get<ApiResponse<CompanyMarket[]>>(`/market-data/${companyId}`);
  return response.data.data ?? [];
}
